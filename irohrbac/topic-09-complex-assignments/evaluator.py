...
def evaluate(ast, environment, watchdog=None):
    global __watchdog_identifier
    if watchdog is not None:
        __watchdog_identifier = watchdog
    ...
    if ast["tag"] == "assign":
        ...
        if __watchdog_identifier is not None:
            if target["tag"] == "identifier" and target["value"] == __watchdog_identifier:
                # Log for simple assignments
                # print(f"DEBUG: Checking WATCHDOG for {target['value']} with value {value}")
                line_num = ast.get("position", "unknown")
                print(f"WATCHDOG: {__watchdog_identifier} = {value} at line {line_num}")
            elif target["tag"] == "complex":
                # Log for complex assignments
                base_str = ast_to_string(target["base"])
                index_str = ast_to_string(target["index"])
                # print(f"DEBUG: Checking WATCHDOG for {base_str}[{index_str}] with value {value}")
                line_num = ast.get("position", "unknown")
                print(f"WATCHDOG: {base_str}[{index_str}] = {value} at line {line_num}")
